...
要验证 apt 路径本身，跑 `make verify-apt`：它在一次性的 Debian 与 Ubuntu 容器里跑真实 apt（需要 Docker 与网络）。
...
`flock` 是 Linux 自带、macOS 没有的命令，内核锁行为无法在 macOS 上模拟，因此跳过而不是失败。这两个用例由流水线在 Linux 上执行。

仓库里没有 `config.sh` 时还会多跳过一条（公开仓库的正常状态）：
...
注意 shfmt 的排版规则随版本变化，发行版仓库里的版本常与开发者本机不一致。CI 因此安装固定版本；本机与 CI 结论不一致时，先比对 `shfmt --version` 与 `.github/workflows/ci.yml` 里固定的版本。
