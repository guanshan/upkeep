...
## 用 mock 测试，再用 doctor 补真实契约，apt 再加一层容器验证

测试全部基于 mock 命令，不触碰真实包管理器。`make doctor` 是独立的只读体检，核对真实工具的版本与关键前提。`make verify-apt` 针对 apt 再往前走一步。
...
apt 又是另一种情况：最容易出问题的几点——sudo 会不会接受 `--preserve-env`、debconf 会不会把无人值守的升级卡住、真实升级在 `--no-remove` 加 `--force-confold` 下能否完成——在 macOS 开发机上根本观察不到。`make verify-apt` 在一次性的 Debian 与 Ubuntu 容器里用真实 apt 跑被测脚本，覆盖 root 与非 root 经 sudo 两条路径，并强制制造一次真实的软件包升级。它需要 Docker 与网络，所以不属于 `make test`。
...
代价是若干写法比较啰嗦，具体约束见 [架构说明](architecture.zh-CN.md) 末节。
